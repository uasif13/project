/*
  astar mpi cuda
  compile: make
  run: mpirun -n <nprocs> astar <no_of_nodes> <start_node> <end_node> <percent>
 */
#include <igraph.h>
#include <mpi.h>
#include <cmath>
...
void output_tail(uint64_t* graph, int graph_size, int tail_count) {
     uint64_t no_of_nodes = graph_size*graph_size;
     for (int i = tail_count ; i >= 1; i--) {
     	 printf("%*" PRIu64  "", 3, graph[no_of_nodes-i]);
     }
     printf("\n");
}
void output_head(uint64_t* graph, uint64_t head_count) {
     for (int i = 0 ; i < head_count; i++) {
     	 printf("%*" PRIu64  "", 3, graph[i]);
     }
     printf("\n");
}
...
int main(int argc, char * argv[]) {
  ...
  while (checkNVV(checkBuf, nprocs)) {
    ...
    //    output_head(openList, 5);
    
    astar_cuda(openList, closedList, src, dst, gList, hList, parent, nVVBuf,  my_rank, my_work, total_src, total_dst,grid_size, no_of_nodes, nprocs, heuristic_type, end, iteration);
   ...
   //   if (my_rank == 1)
   //output_tail(buffer_g_recv, grid_size, 5);
